...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	vector< vector <float> > newGrid;
      ...
      for (int j=0;j<grid[0].size();j++){
        placeholder.push_back(1.0/(grid.size()*grid[0].size())
      }
      newGrid.push_back(placeholder);
    }
  ...
  float blurring) 
{
  ...
  for (int i=0;i<beliefs.size();i++){
    ...
    for (int j=0;j<beliefs[0].size();j++){
      ...
      int new_i = remainder((i + dy),width)
      int new_j = remainder((j + dx),width);
	...
	float p_miss) 
{
	vector< vector <float> > newGrid;
      ...
      for (int j=0;j<grid[0].size();j++){
        ...
        empty_list.push_back(beliefs[i][j] * (hit * p_hit + (1-hit) * p_miss));
     
      }
      
      newGrid.push_back(empty_list);
      ...
      for (int j=0;j<newGrid[0].size();i++){
        s += newGrid[i][j];
      }
    }
