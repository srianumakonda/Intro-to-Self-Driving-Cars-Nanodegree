...
vector< vector<float> > normalize(vector< vector <float> > grid) {
  ...
  for (int i=0;i<grid.size();i++){
      placeholder.clear();
      for (int j=0;j<grid[0].size();j++){
        placeholder.push_back(grid[i][j]/total);
      }
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
    ...
    vector < vector <float> > window = {{corner_prob,  adjacent_prob,  corner_prob},{adjacent_prob, center_prob,  adjacent_prob},{corner_prob,  adjacent_prob,  corner_prob}};
      ...
      for (int j=0;i<width;j++){
        empty_arr.push_back(0.0);
      }
      newGrid.push_back(empty_arr);
    }
      ...
      for (int j=0;j<width;j++){
        for (int dx=-1;dx<2;dx++){
          for (int dy=-1;dy<2;dy++){
            ...
            int new_i = remainder((i + dy),height);
            int new_j = remainder((i + dx),width);
